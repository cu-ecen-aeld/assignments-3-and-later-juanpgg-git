...
struct thread_data {
    ...
    int wait_to_release_ms;  // Time in milliseconds to wait before releasing the mutex
    pthread_mutex_t* mutex;  // Pointer to the mutex
    bool thread_complete_success; // Boolean flag to indicate if the thread completed successfully
};
